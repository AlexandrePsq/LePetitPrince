...
if __name__ == '__main__':
        ...
        for index_mask in range(len(labels)-1):
            ...
            for analysis in analysis_parameters['scatter_plots']:
                for subject in subjects:
                    ...
                    model1 = [os.path.join(paths.path2derivatives, source, analysis['input_data_folder1'], language, subject, analysis['model1_folder'], analysis['model1']+ '_' + subject + '.nii.gz')]
                    model2 = [os.path.join(paths.path2derivatives, source, analysis['input_data_folder2'], language, subject, analysis['model2_folder'], analysis['model2'] + '_' + subject + '.nii.gz')]
                    ...
                    plt.scatter([np.percentile(x, 25)], [np.percentile(y, 25)], c='blue', label='Median value')
                    plt.scatter([np.percentile(x, 50)], [np.percentile(y, 50)], c='yellow', label='Median value')
                    plt.scatter([np.percentile(x, 75)], [np.percentile(y, 75)], c='red', label='Median value')
    ...
    if 'model_comparison' in args.analysis[0]:
        ...
        df_pearson_final.to_csv(os.path.join(save_folder_pearson, 'pearson_data.csv'), index=False)
        df_r2_final.drop_duplicates(inplace=True)
        df_r2_final.to_csv(os.path.join(save_folder_r2, 'r2_data.csv'), index=False)
        df_significant_pearson_final.drop_duplicates(inplace=True)
        df_significant_pearson_final.to_csv(os.path.join(save_folder_significant_pearson, 'significant_pearson_data.csv'), index=False)
        df_significant_r2_final.drop_duplicates(inplace=True)
        df_significant_r2_final.to_csv(os.path.join(save_folder_significant_r2, 'significant_r2_data.csv'), index=False)