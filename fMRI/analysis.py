        ...
        for index_mask in range(len(labels)-1):
            ...
            for analysis in analysis_parameters['scatter_plots']:
                for subject in subjects:
                    ...
                    plt.scatter([np.percentile(x, 25)], [np.percentile(y, 25)], c='blue', label='First quartile')
                    plt.scatter([np.percentile(x, 50)], [np.percentile(y, 50)], c='yellow', label='Median ')
                    plt.scatter([np.percentile(x, 75)], [np.percentile(y, 75)], c='red', label='Third quartile')
