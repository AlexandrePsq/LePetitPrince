...
def forward(self, input, hidden, param, mask=None):
    ...
    return input, (torch.cat([last_gates[i]['hidden'].unsqueeze(0) for i in range(param['nlayers'])], 0), torch.cat([last_gates[i]['cell'].unsqueeze(0) for i in range(param['nlayers'])], 0))