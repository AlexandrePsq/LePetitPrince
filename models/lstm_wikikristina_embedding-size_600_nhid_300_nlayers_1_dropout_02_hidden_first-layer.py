...
def load():
    ...
    mod = model.RNNModel('LSTM', 5, 600, 300, 1, dropout=0.2) # ntoken is chosen randomly, it will or has been determined during training
...
def generate(model, run, language, textgrid, overwrite=False):
    ...
    save_all = None
    model.param = {'rnn_type':'LSTM', 'ntoken':50001, 'ninp':600, 'nhid':300, 'nlayers':1, 'dropout':0.2, 'tie_weights':False}
...
if __name__ == '__main__':
    ...
    params = Params()
    paths = Paths()
    mod = model.RNNModel('LSTM', 5, 600, 300, 1, dropout=0.2)
